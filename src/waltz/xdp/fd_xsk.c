...
fd_xsk_mmap_ring( fd_xdp_ring_t * ring,
  ...
  /* Lock the (4KB page) mappings in DRAM to prevent swapping.
     Also advise the kernel to not dump this region for consistency
     with fd_shmem. Reimplements syscall logic of fd_numa_mlock()
     from fd_shmem_private.h to circumvent the ASan interceptor
     while avoiding including a private header. */ 
